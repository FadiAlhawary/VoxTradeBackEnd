...
namespace VoxTrade.Services.Implementation
{
    public class WalletRepo : IWalletRepo
    {
        ...
        public async Task<WalletDto> GetWallet(int userId,bool WithHisory)
        {
            try
            {
                const string sql =
    ...
    """;
                ...
                if (WithHisory)
                {
                    var history = await _historyRepository.GetWalletHistory(userId);
                    if (history != null)
                        wallet.walletHistory = history;
                }
